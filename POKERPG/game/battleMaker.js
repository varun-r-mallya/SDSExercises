...
export var battleMaker = {
    start: function (canvas, keydownHandler) {
        ...
        setTimeout(function() {Music.stopOpening();}, 2000);
        Music.stopStarter
        Music.stopSearch();
        Music.playBattle();
    ...
    drawScene: function(canvas, pokemonData, vsData) {
        ...
        let shakeInterval = setInterval(function() {
            ...
            context.fillText(pokemonData.name.slice(0, 12), 150 + shake, 450 + shake);
            context.fillText(vsData.name.slice(0,12), 675 + shake, 450 + shake);
            shake = shake === 0 ? shakeAmount : 0;
        }, 100);
        setTimeout(function() {
            clearInterval(shakeInterval);
        }, 2000);
        setTimeout(function () { realBattle.start(canvas, pokemonData, vsData) }, 3000);
